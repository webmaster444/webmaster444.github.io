...
Celestial.display = function(config) {
  ...
  prjMap = Celestial.projection(cfg.projection).rotate(rotation).translate([(width+200)/2, height/2]).scale(scale);
    
  var canvas = d3.select(par).selectAll("canvas");
  if (canvas[0].length === 0) canvas = d3.select(par).append("canvas").attr('id','mapcanvas');
  var canHeight = height + height / 2;
  canvas.attr("width", width+200).attr("height", canHeight);

  var context = canvas.node().getContext("2d");  
  context.fillStyle = '#000';
  context.fillRect(0, 0, width, height + height / 2);
  ...
  context.font = 'italic 1em Calibri';
  context.fillText('Hello World!', 150, 100);

  context.fillText(config.date,width/2,height + 50);
  context.fillText(config.date,width/2,height + 60);
  context.fillText(config.date,width/2,height + 70);
  context.fillText(config.date,width/2,height + 80);
  context.fillText(config.date,width/2,height + 90);
  context.fillText(config.date,width/2,height + 100);
  ...
  function reproject(config) {
    ...
    d3.select({}).transition().duration(interval).tween("projection", function () {
      return function(_) {
        ...
        height = width/ratio;
        var canHeight = height + height / 2;
        canvas.attr("width", width).attr("height", canHeight);
